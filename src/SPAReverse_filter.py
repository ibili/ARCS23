...
def cdbg_read(f):
    ...
    for line in ifilter(lambda x: len(x)>0, imap(string.strip, f)):
        if state == 0:
            print "liyanbo"
            m = re.match(r'>NODE_(\d+)_length_(\d+)_cov_(\d+.{0,1}\d+)_ID_(\d+)', line)
            if m:
                copy_num = int(float(m.group(3)))
                state = 1
            else:
...
def cdbg_read(f):
    ...
    while line:
        ...
        if len(cols) > 2:
            if len(seq) > 0:
                #print "read contigs.fasta; contigId, seq, copy_num: ", contigId, seq, copy_num
                yield contigId, seq, copy_num
            state = 0
            seq = ""
        if state == 0 and len(cols) > 2:
            contigId = int(cols[1])
            copy_num = float(cols[5]) 
            state = 1
...
if __name__ == '__main__':
    ...
    # copy num decide in SPAGetUnique
